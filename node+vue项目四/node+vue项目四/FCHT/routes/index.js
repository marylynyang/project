...
var recUpdate = function (req, res, collections, data) {
    if (data.length === 0) {
        ...
        handler(req, res, collections, selectors, function (result) {
...
router.post('/AdminLoginAndRegHandler', function (req, res) {
    ...
    switch (action) {
        case "login": // 登录
            ...
            handler(req, res, "administors", {userName: req.body.userName, password: password}, function (data) {
                if (data.length === 0) {
        ...
        case "add": // 添加管理员
            ...
            handler(req, res, "administors", {userName: req.body.userName}, function (docs) {
                if (docs.length != 0) {
                    res.send({err: "用户名已存在，请从新输入"});
                } else {
                    handler(req, res, "administors", null, function (arr) {
                        var md5 = crypto.createHash('md5');
                        //组织用户信息并作一些校验
                        var userInfos = {};
                        ...
                        if (userInfos.phone != false && userInfos.userName != false && userInfos.turename != false) {
                            handler(req, res, "administors", userInfos, function (data) {
                                if (data.result.n != 1) {
    ...
    switch (action) {
        case "show": // 管理员显示
            handler(req, res, "administors", null, function (arr) {
                ...
                handler(req, res, "administors", selector, function (data) {
                    if (data.length == 0) {
        ...
        case "delete": // 管理员删除
            //删除操作
            handler(req, res, "administors", {tokenId: parseInt(req.query.tokenId), isdelete: true}, function (data) {
                //console.log(data);
                if (data.result.n == 0) {
                    res.send({"err": "删除失败"});
                } else {
                    ...
                    handler(req, res, "administors", {tokenId: {$gt: parseInt(req.query.tokenId)}}, function (da) {
        ...
        case "lockuser": // 学员的冻结
            ...
            handler(req, res, "studentList", {tokenId: parseInt(req.query.tokenId)}, function (da) {
                ...
                if (da.length == 0) {
                    ...
                    //切换当前字段isstate的状态
                    handler(req, res, "studentList", selectors, function (result) {
                        if (result.result.n == 1) {
...
router.post('/AdminHandler', function (req, res) {
    ...
    switch (action) {
        case "update": //修改管理员信息
            ...
            handler(req, res, "administors", selectors, function (data) {
                if (data.result.n == 0) {
        ...
        case "returnuserinfo": // 返回登录管理员信息
            ...
            handler(req, res, "administors", {"_id": sessionId}, function (data) {
